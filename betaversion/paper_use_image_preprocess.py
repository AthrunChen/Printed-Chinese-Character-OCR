...
def sege_func(filename="IMG_20180424_123611_HHT.jpg", dst_dir=r"D:\build my dl NN test\imagegen", base_dir=r"D:\build my dl NN test\imagedataset", temppicfile=r"D:\build my dl NN test\tempfile"):
    ...
    scale = image_color.shape[1]/2000
    ...
    cv2.threshold(image, 85, 255, 1, image)
    ...
    def extract_peek_ranges_from_array_vertical(array_vals, minimun_val=200, minimun_range=50, gate=3000, bias=0,  upergate=10, beta = 0.6):
        start_i = None
        end_i = None
        peek_ranges = []
        beta = beta
    ...
    for j, file in enumerate(index[:count]):
        ...
        if j == 0:
            ...
            peek_ranges_vertical = extract_peek_ranges_from_array_vertical(vertical_sum,minimun_val=5, minimun_range=10, gate=2, bias=5, upergate=60, beta=0.61)
            cutImage_vertical(image_block, j, peek_ranges_vertical, dst_dir1=dst_dir+r'\address')

    print("segementate completed!")


sege_func(filename="0.jpg")
